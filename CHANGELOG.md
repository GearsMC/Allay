...
- Added MCBE 1.26.50 client connection support (protocol v2193 for the release build, with v2192 kept for the last preview; both share the same wire mappings).
...
- Fences, glass panes, iron and copper bars, tripwire and stairs now compute their 1.26.50 connection and corner states when placed (by players, pistons or `setBlockState`) and when a horizontal neighbour changes. The rules follow vanilla as measured on BDS (`tools/vanilla-oracle`), including which faces of every block state a fence or pane attaches to (`block_connection_faces.json`), and deliberately differ from PocketMine in five places documented on `BlockConnectionRules`. Worlds saved before 1.26.50 are fixed once, offline, with `./gradlew :server:migrateConnections -Pworlds=<dir>[,<dir>] [-PdryRun=true]`, which rewrites only the chunk sections whose stairs or connection blocks change.
