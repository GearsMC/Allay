...
public record CustomItemDefinition(
    ...
    public CustomItemDefinition {
        ...
        rawProperties = immutableCopyOfProperties(rawProperties);
        rawComponents = immutableCopy(rawComponents);
    }

    /**
     * GearsMC fork: {@code item_properties} girdileri icin ayri kopyalayici.
     *
     * <p>Ozellikler duz degerdir — int, float, byte, string — bilesik (compound) degil.
     * Bu yuzden {@code rawProperties} {@code Map<String, NbtMap>} degil
     * {@code Map<String, Object>} tutar. Ornegin alet olarak kaydedilmeyen ama elde
     * tutuluyormus gibi gorunmesi gereken {@code core:hammer}
     * {@code hand_equipped = 1b} gecer; NbtMap'e sarilamaz.</p>
     *
     * @param values kopyalanacak ozellikler
     * @return degistirilemez kopya; {@code null} girdide bos harita
     */
    private static Map<String, Object> immutableCopyOfProperties(Map<String, Object> values) {
        if (values == null) {
            return Map.of();
        }
