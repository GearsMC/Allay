...
2. Dump the canonical block palette from the game as well:
   `python3 tools/vanilla-oracle/run_oracle.py --mode palette --bds-version <version> --output
   data/resources/unpacked/staging-<version>/oracle_block_palette.json`. The palette is never sent over the network,
   so it used to come from CloudburstMC/Data; the oracle builds it from Mojang's own block metadata (property names)
   plus a measurement of each block's accepted values, and for 26.50 it reproduced CloudburstMC's 22091 states exactly.
...
| `block_palette.nbt`         | From Endstone, or from the vanilla oracle (`--mode palette`) via `importBedrockData`                                      |
